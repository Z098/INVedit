...
namespace INVedit
{
    public partial class EnchantForm : Form
    {
        ...
        public void Update(ItemSlot slot)
        {
            ...
            else
            {
                ...
                foreach (var kvp in Data.enchantments)
                {
                    ...
                    if (boxAllow.Checked || enchantment.items.Contains(tag["id"] + ""))
        ...
        void SelectEnchantment(string id)
        {
            ...
            editId.Value = decimal.Parse(id); editId.Text = id.ToString();
            if (lastEnchantment != null)
                boxName.Items.Remove(lastEnchantment);
            if (enchantment == null)
            {
                lastEnchantment = new Data.Enchantment(id, "Unknown enchantment " + id, 0, new List<string>());
        ...
        void ChangeLevel(short level)
        {
            ...
            if (level != 0)
            {
                ...
                foreach (var ench in slot.Item.tag["tag"]["ench"])
                    if (ench["id"] + "" == enchantment.id) { tag = ench; break; }
                if (tag == null)
                {
        ...
        {
            SelectEnchantment(editId.Value + "");
        }
        ...
        void BoxAllowCheckedChanged(object sender, EventArgs e)
        {
            ...
            Data.Enchantment enchantment = null;
            if (Data.enchantments.ContainsKey(editId.Value + ""))
                enchantment = Data.enchantments[editId.Value + ""];
